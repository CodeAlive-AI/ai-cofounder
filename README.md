...
![CEO AI OS Banner](banner.png)

**Your AI Chief of Staff for startup operations.** 73 skills that turn any AI agent into an augmented CEO: strategic reviews, hypothesis tracking, GTM playbooks, founder sales, live pipeline operations, and more.
...
**Technical founders** — honestly, this is a must-have. You're good at building product but probably underinvesting in strategy, sales, and marketing. These 73 skills fill that gap: structured frameworks for everything from weekly CEO reviews to cold email outreach, plus live pipeline operations (signal scanning, signup monitoring, stargazer research) — without needing to hire a Chief of Staff or a sales coach. If you catch yourself hiding in code instead of talking to customers, this system will call you out.
...
```bash
npx skills add CodeAlive-AI/ceo-ai-os -g
```

Or install individual skills:

```bash
npx skills add CodeAlive-AI/ceo-ai-os@hypothesis-tracker -g
npx skills add CodeAlive-AI/ceo-ai-os@decision-playbook -g
npx skills add CodeAlive-AI/ceo-ai-os@founder-sales -g
```

> **No terminal?** Just tell your agent: *"Install skills from https://github.com/CodeAlive-AI/ceo-ai-os following the README"*
...
73 skills organized into a CEO operating system. Each skill is a structured workflow with frameworks from 100+ product leaders. They work standalone, but together they form a system:
...
```
ceo-ai-os/
├── skills/                  <- 73 portable skills (work in any agent)
│   ├── ceo-weekly-review/
│   ├── hypothesis-tracker/
│   ├── founder-sales/
│   ├── signal-scanner/
...
|----------|:-----:|---------|
| **CEO Operating System** | 15 | Weekly reviews, hypothesis tracking, decision playbooks, strategic analysis, focus-keeper, morning/evening cadence |
| **Go-To-Market** | 25 | Positioning, founder sales, cold email, SEO, content strategy, pricing, user onboarding, positioning workshop |
| **Research & People** | 4 | Company intel, competitor research, person lookup, social listening |
| **Sales & Pipeline** | 13 | Lead enrichment, find-email-by-name, sales prospecting, pipeline manager, signal scanner, signup monitor, stall rescue, value-moment email, github stars monitor, outreach sender, meeting prep, feedback tracker, PR opportunity scanner |
| **Launch** | 2 | Product Hunt launch playbook + 30-day timeline |
| **Writing Quality** | 2 | AI pattern removal (humanizer), prompt engineering |
| **Tools** (optional) | 12 | PostHog analytics, Exa research, Grafana monitoring, Google Calendar, Orthogonal, Composio, web browser |

<details>
<summary><strong>Full skill catalog (click to expand)</strong></summary>

### CEO Operating System (15 skills)
...
| `morning-brief` | Morning CEO brief: overnight report + pre-staged GTM action + status line |
| `evening-recap` | Evening CEO recap: today's actions, pipeline changes, belief updates |
| `focus-keeper` | Core behavioral engine — keeps the founder focused on constraint work |
...
| `positioning-workshop` | Full structured positioning exercise with competitive research and team alignment |
...
### Research & People (4 skills)

| Skill | What it does |
|-------|-------------|
| `company-intel` | Full company intelligence report: overview, team, funding, products, news |
| `competitor-research` | Deep competitor analysis: products, pricing, team, funding, strategy, positioning |
| `person-lookup` | Look up a person's background: work history, education, social profiles |
| `social-listening` | Monitor brand mentions, competitor activity, and industry conversations |

### Sales & Pipeline (13 skills)

| Skill | What it does |
|-------|-------------|
| `lead-enrichment` | Enrich leads with email, phone, company data |
| `find-email-by-name` | Find a professional email given a name and company/domain |
| `sales-prospecting` | Build targeted prospect lists with verified contacts matching your ICP |
| `pipeline-manager` | Manage sales pipeline: add/update prospects, score, track stage transitions |
| `signal-scanner` | Scan for buying signals across ICP companies (7 signal types) |
| `signup-monitor` | Cron: query PostHog for new signups, research, alert when interesting |
| `stall-rescue` | Cron: detect users who signed up but didn't activate, draft personalized email |
| `value-moment-email` | Cron: when a user gets first value, draft personalized founder email |
| `github-stars-monitor` | Cron: track who stars/forks your repos, research notable engagement |
| `outreach-sender` | Send outreach emails and manage follow-up sequences |
| `meeting-prep` | Prepare intelligence briefings before sales meetings |
| `feedback-tracker` | Track outreach outcomes: sent → replied → meeting → learning |
| `pr-opportunity-scanner` | Scan for free PR opportunities — community threads, roundup articles, conferences |
...
### Tools (12 skills, optional)

These skills connect the agent to external services via MCP. In agents with native MCP support (OpenClaw 2026.4+, Claude Code, Claude Cowork, Cursor, etc.), tools are called directly as MCP functions — no CLI bridge needed. Configure the API keys in your agent's MCP settings — see [docs/adding-tools.md](docs/adding-tools.md).

| Skill | What it does | Requires |
|-------|-------------|----------|
| `posthog-analytics` | Query PostHog data | PostHog account + MCP server |
| `grafana-monitoring` | Grafana dashboards, Loki logs, Prometheus metrics, Tempo traces | Grafana Cloud + MCP server |
| `exa-company-research` | Research companies via Exa | Exa API key + MCP server |
| `exa-people-research` | Research people and LinkedIn profiles | Exa API key + MCP server |
| `exa-personal-site` | Find personal sites and blogs | Exa API key + MCP server |
| `exa-x-search` | Search X/Twitter posts | Exa API key + MCP server |
| `exa-code-search` | Search code repositories | Exa API key + MCP server |
| `exa-lead-gen` | Deep search for lead generation | Exa API key + MCP server |
| `google-calendar` | Create/list/manage Google Calendar events | Google account + MCP server |
| `orthogonal` | Access premium APIs and curated skills via Orthogonal marketplace | Orthogonal account |
| `composio-cli` | Operate the Composio CLI to find tools, connect accounts, execute actions | Composio account |
| `web-browser` | Browse web pages via agent-browser | [agent-browser](https://github.com/nichochar/agent-browser) CLI |

The other 61 skills work out of the box with no external dependencies.
