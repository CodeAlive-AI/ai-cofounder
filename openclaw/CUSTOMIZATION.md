...
The power of CEO AI OS comes from deep customization to your company. A generic setup is 10% as useful as a properly configured one.
...
If you use PostHog, Exa, Grafana, or other tools via MCP, configure them here (your agent will call them as `<server>__<tool>` once the MCP server is wired up). Otherwise, leave TOOLS.md minimal — the CEO OS works without external tools.
