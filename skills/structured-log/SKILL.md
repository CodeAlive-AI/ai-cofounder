...
```markdown
### 14:30 — [TOOL] PostHog: 41% activation step failures

PostHog query revealed a high percentage of errors during the key activation step.

**Facts:**
- 41% of `primary_resource_action` events have `processingStatus=failed` (30d)
- Main errors: timeout on large inputs (above the documented size limit)
- Affects the `first_primary_resource_added -> primary_resource_processed` funnel stage

**Concepts:** activation-blocker, funnel-drop, PostHog, TTFV
**Files:** —
```
